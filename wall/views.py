...
def new_post(request):
    if request.method == "POST":
        ...
        form = PostForm()
    return render(request, 'wall/post_edit.html', {'form': form, 'title': 'New post'})
...
def post_edit(request, pk):
    ...
    if post.author == request.user:
        ...
        return render(request, 'wall/post_edit.html', {'form': form, 'title': post.title})
    else:
        messages.success(request, 'You cant edit this post!')
        return redirect('current_post', pk=post.pk)
...
def show_profile_posts(request, user_id):
    author = User.objects.get(id=user_id)
